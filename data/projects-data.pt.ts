...
const projects: Project[] = [
  {
    ...
    longDescription:
      "Aplicação fullstack para gerenciamento financeiro pessoal, com backend em Node.js, TypeScript e PostgreSQL e frontend em desenvolvimento utilizando React, Vite, Redux e TanStack Query. O projeto foi arquitetado aplicando arquitetura hexagonal e princípios SOLID, garantindo alta escalabilidade, manutenibilidade e testabilidade. A arquitetura prioriza desacoplamento total entre camadas (Controllers → Services → Repositories), permitindo evolução independente de cada componente e facilitando testes unitários e de integração. Implementei estratégia robusta de testes com Factories e Fixtures, atingindo alta cobertura de código via Jest e garantindo confiabilidade nas entregas. O padrão de Adapters isola dependências externas, permitindo substituições sem impacto na lógica de negócio. Em termos de segurança, a aplicação utiliza hash de senhas, validação rigorosa em múltiplas camadas com Zod (type-safe) e constraints no banco de dados para garantir integridade dos dados. O pipeline de qualidade inclui ESLint, Prettier e Husky com validação pré-commit, além de commits semânticos padronizados. O ambiente foi completamente dockerizado para facilitar onboarding e garantir consistência entre dev/test/prod. A estrutura monorepo está preparada para escalar, integrando backend, frontend web e futura versão mobile iOS. O roadmap prevê implementação de autenticação JWT, logs estruturados com Pino, observabilidade com OpenTelemetry, sistema de cache para otimização de performance em rotas críticas, e CI/CD completo com GitHub Actions para entregas automatizadas.",
  ...
  },
  {
    ...
    longDescription:
      "Aplicação fullstack desenvolvida com Next.js 15, React 19 e TypeScript para gerenciamento de clínicas médicas com suporte a multi-tenancy, permitindo que usuários administrem múltiplas clínicas mantendo isolamento e segurança dos dados. O sistema implementa controle completo de agendamentos, assinaturas via Stripe e autenticação moderna com BetterAuth (sessões server-side). A arquitetura foi construída utilizando o App Router do Next.js, aplicando Server Actions para operações de backend e separação clara de responsabilidades entre componentes, lógica de dados (data layer) e ações do servidor. O projeto utiliza Drizzle ORM com PostgreSQL para garantir type-safety em toda a camada de persistência, validação rigorosa com Zod e React Hook Form, além de interface moderna construída com Tailwind CSS e shadcn/ui. Em termos de segurança, o sistema implementa autenticação baseada em sessões HTTP-only (ao invés de JWT), proporcionando maior proteção contra ataques XSS. A integração com Google OAuth facilita onboarding de usuários, enquanto a integração completa com Stripe (webhooks, customer portal) garante gestão segura de assinaturas e pagamentos recorrentes. O modelo multi-tenant permite que um único usuário gerencie diversas clínicas com isolamento total de dados, garantindo conformidade e privacidade. O sistema oferece observabilidade completa do fluxo de agendamentos, com confirmação manual de consultas e controle granular de permissões por clínica. Desenvolvido como projeto de estudos para aprofundar conhecimentos em arquitetura moderna de aplicações React, autenticação baseada em sessão, integrações de pagamento e design multi-tenant escalável.",
  ...
  },
  {
    ...
    longDescription:
      "Aplicação fullstack desenvolvida com foco em comunicação bidirecional em tempo real utilizando WebSocket via Socket.io e MongoDB. O projeto implementa um editor de documentos colaborativo (estilo Google Docs), onde múltiplos usuários podem editar simultaneamente com sincronização instantânea de alterações. A arquitetura backend explora profundamente o funcionamento do protocolo WebSocket, estabelecendo conexões persistentes entre cliente e servidor para troca de mensagens bidirecionais com latência mínima. A implementação com Socket.io gerencia automaticamente reconexões, fallback para polling e broadcasting eficiente para múltiplos clientes conectados. O sistema utiliza MongoDB para persistência de documentos, aproveitando sua flexibilidade para armazenar operações de edição e sincronizar estado entre usuários. A aplicação implementa CRUD básico de documentos com atualização em tempo real, garantindo que todas as alterações sejam propagadas instantaneamente para todos os usuários ativos. O projeto demonstra compreensão prática de conceitos fundamentais de aplicações real-time: gerenciamento de conexões simultâneas, broadcasting de eventos, sincronização de estado distribuído e tratamento de desconexões/reconexões. A arquitetura baseada em eventos permite escalabilidade e facilita adição de novas funcionalidades colaborativas. Desenvolvido como projeto de estudos para dominar comunicação em tempo real, protocolos WebSocket, Socket.io e bancos NoSQL (MongoDB) — tecnologias essenciais para construção de aplicações colaborativas, chats, dashboards ao vivo e sistemas que exigem baixa latência e sincronização instantânea.",
  ...
  },
  {
    ...
    longDescription:
      "Aplicação frontend desenvolvida com React e CSS Modules, inspirada no Instagram e voltada para compartilhamento de fotos de cachorros. Projeto que consolidou fundamentos essenciais de React moderno, explorando componentização, custom hooks, gerenciamento de estado e otimização de performance para garantir excelente experiência do usuário (UX). A arquitetura de componentes foi estruturada seguindo princípios de reutilização e separação de responsabilidades, com hooks personalizados para abstrair lógica complexa (autenticação, requisições HTTP, validação de formulários). A aplicação utiliza React Hooks nativos (useState, useEffect, useContext, useCallback) de forma otimizada, evitando re-renderizações desnecessárias e garantindo fluidez na interface. O projeto implementa lazy loading de imagens, skeleton screens durante carregamentos e transições suaves, priorizando perceived performance e feedback visual constante ao usuário. A estilização modular com CSS Modules proporciona encapsulamento de estilos e evita conflitos de nomenclatura, mantendo o código escalável. A aplicação consome uma API REST externa com tratamento robusto de erros, loading states e fallbacks, demonstrando boas práticas de integração com backends. Testes automatizados com React Testing Library garantem confiabilidade dos componentes críticos e facilitam refatorações futuras. Desenvolvido como projeto fundamental no início da carreira para dominar React essencial, padrões de componentização, custom hooks, otimização de performance e design centrado no usuário — competências base que sustentam o desenvolvimento de aplicações frontend profissionais e escaláveis.",
