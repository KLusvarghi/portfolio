...
import { Github, Linkedin, Mail, MapPin, Instagram, Copy, Check } from "lucide-react"
...
export default function ContactPage() {
  ...
  const github = resumeData?.personalInfo?.github || "https://github.com/KLusvarghi"
  const linkedin = resumeData?.personalInfo?.linkedin || "https://www.linkedin.com/in/kaua-lusvarghi-fullstack-dev/"
  const instagram = resumeData?.personalInfo?.instagram || "https://www.instagram.com/lusvarghkaua"
    ...
    <div className="container py-8 md:py-12">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8 md:mb-12 text-center">
          <h1 className="text-3xl md:text-5xl font-bold mb-3 md:mb-4">{t.contact.heading}</h1>
          <p className="text-base md:text-xl text-muted-foreground max-w-2xl mx-auto">{t.contact.description}</p>
        </div>
        ...
        <div className="space-y-4 md:space-y-6">
            ...
            <Card className="bg-gradient-to-br from-blue-500/5 to-cyan-500/5 border-blue-500/20 hover:border-blue-500/40 transition-colors">
              <CardContent className="p-4 md:p-6">
                <div className="flex flex-col items-center text-center gap-3 md:gap-4">
                  ...
                  <div>
                    <p className="font-semibold mb-1 text-sm md:text-base">{t.contact.email}</p>
                    <button
                      onClick={copyToClipboard}
                      className="text-xs md:text-sm text-zinc-400 hover:text-white transition-colors flex items-center gap-1 md:gap-2 group"
                    >
                      <span className="break-all">{email}</span>
                      {copied ? (
                        <Check className="h-3 w-3 md:h-4 md:w-4 text-green-500 flex-shrink-0" />
                      ) : (
                        <Copy className="h-3 w-3 md:h-4 md:w-4 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
                      )}
                    </button>
                  </div>
                </div>
            ...
            <Card className="bg-gradient-to-br from-blue-500/5 to-cyan-500/5 border-blue-500/20 hover:border-blue-500/40 transition-colors">
              <CardContent className="p-4 md:p-6">
                <div className="flex flex-col items-center text-center gap-3 md:gap-4">
                  ...
                  <div>
                    <p className="font-semibold mb-1 text-sm md:text-base">{t.contact.location}</p>
                    <p className="text-xs md:text-sm text-zinc-400">{location}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <CardContent className="p-6 md:p-8">
              <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6">
                ...
                <div className="flex-1 text-center md:text-left">
                  <h3 className="text-lg md:text-xl font-bold mb-2">{t.contact.whatsappHeading}</h3>
                  <p className="text-sm md:text-base text-muted-foreground mb-4">{t.contact.whatsappDescription}</p>
          ...
          <div className="text-center py-6 md:py-8">
            <p className="text-muted-foreground mb-4 md:mb-6 text-sm md:text-base">{t.contact.findMeOn}</p>
